		...
		if (args[0].equals("remove")) {
			if (sender instanceof Player p) {
				if (args.length < 4) {
					...
					BreakFurniture(new Location(p.getLocation().getWorld(), Double.parseDouble(args[1]),
		...
		if (!event.isCancelled()) {
			if (block.getType() == Material.BARRIER) {
				List<Entity> nearbyEntites = (List<Entity>) block.getWorld().getNearbyEntities(
						block.getLocation().add(0, 1, 0), 0.13, 0.2, 0.13);
				for (Entity nearbyEntity : nearbyEntites) {
					if (nearbyEntity instanceof ItemFrame frame) {
						if (frame.getItem().getType() == Material.OAK_PLANKS) {
							main.getConfig().getConfigurationSection("Furniture").getKeys(false).forEach(key -> {
								if (main.getConfig().getInt(
										"Furniture." + key + ".custommodeldata") == frame.getItem().getItemMeta().getCustomModelData()) {
